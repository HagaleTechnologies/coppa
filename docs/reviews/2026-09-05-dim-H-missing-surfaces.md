...
| 8 | Bearer token for WS/HTTP when `bind_address` ≠ loopback (`[host] auth_token`, sent as first message or `?token=`) | api-prereq | H | S | `coppad.toml.example:33-37` warns of unauthenticated PTT |
