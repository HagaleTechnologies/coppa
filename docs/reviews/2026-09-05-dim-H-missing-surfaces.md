...
| Beacon / station-ID RX | outer callsign only (`pdu.src`) | `event_loop.rs:2549-2556` `handle_beacon_rx` logs `pdu.src` and payload length only -- it never calls `StationIdPayload::from_bytes` (`mac.rs:279`), so the inner grid and advertised level are NOT currently decoded despite the payload carrying them | **no** (no "heard" list anywhere) | — |
...
**API prerequisites (all `api-prereq` in the hit list):** typed push events for `ptt`, `busy`, `session`, `data` envelope, `heard`, `level`, `log`, `decode_fail` -- these are state transitions and application data, not disposable telemetry, so they need a reliable per-client queue or a latest-value/replay mechanism, NOT the existing lossy broadcast channel (which silently `continue`s on `RecvError::Lagged`, `websocket.rs:311-326` -- a slow dashboard client can miss a `PTT OFF`/`BUSY OFF`/`DISCONNECTED` and never recover it from the polled status snapshot, since not all of those fields are in it); `buffer` and `spectrum` can stay on the existing lossy broadcast since they're disposable/frequently-refreshed. Also: fix the dropped `StatusUpdate` and the raw-string data push; RX level computation in `handle_audio_in`; per-client rate for `spectrum`; HTTP static serving; token/Origin auth required regardless of `bind_address` (loopback binding does not stop a same-machine browser page from opening a cross-origin WebSocket).
...
| 12 | Heard-stations list from beacon RX (callsign, grid, level, SNR, last-heard, count) + WS `heard` event + `/api/heard`. Prerequisite: `handle_beacon_rx` must actually call `StationIdPayload::from_bytes` on the payload first -- today it only logs the outer `pdu.src` and never decodes the inner grid/level fields it carries. | ui | H | S | `event_loop.rs:2549-2556` logs only, no `StationIdPayload::from_bytes` call; `StationIdPayload` has grid/level (`mac.rs:279`); VarAC Last Heard, JS8Call heard list |
| 13 | Spectrum stream: per-client rate (4/8/10 Hz), optional binary frame (128×u8) to cut 1.4 KB JSON/frame, AND profile-aware frequency bounds -- `SPECTRUM_LOW_HZ`/`SPECTRUM_HIGH_HZ` are hard-coded to 300/2800 Hz (`spectrum.rs:27,30`), so on `VHF_FAST` (which occupies ~350-5900 Hz) the dashboard's waterfall would silently hide roughly half the active waveform. Each spectrum frame needs to carry its own current low/high Hz bounds, not assume the fixed HF window. | ui | M | S | `spectrum.rs:25-30,36`, WS probe 31 msgs/8 s |
