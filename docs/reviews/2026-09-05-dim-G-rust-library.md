...
| coppa-protocol | 299 (44) | 12 | 33 | 0 | mixed: `ReceiveError`/`TransmitError` typed, 37 `anyhow` pub fns | no (`serde` is dev-dep only) | **yes** (compiles; `std::time::Instant::now()` in `session.rs:192` will panic at runtime on wasm32-unknown-unknown -- `cp_negotiator.rs` doesn't call `Instant::now()` either, but its public API requires a caller-supplied `Instant`, which a WASM caller has no source for besides the same panicking call) | no; dev-dep cycle with `coppa-bench` |
...
| 12 | `coppa-audio`'s `default = ["cpal-backend", ...]` leaks into its consumers: `coppa-cli` and `coppa-daemon` both pull cpal/alsa transitively even when their own `cpal-backend` feature is off (`coppa-bench` has no `coppa-audio` dependency and is unaffected). This is unwanted dependency activation and a cross-build failure, not a functional no-op -- the consumers still correctly guard their own hardware-audio setup on their local feature, so a daemon built without it still moves no audio; the problem is the wasted compile and the aarch64 build breakage. | deps | H | S | `cargo tree -p coppa-cli -e features` → `coppa-audio ... cpal-backend,default,file-backend`; `cargo check -p coppa-daemon --target aarch64-unknown-linux-gnu` → `failed to run custom build command for alsa-sys v0.4.0` (via `coppa-audio → coppa-daemon`); `crates/coppa-bench/Cargo.toml` has no `coppa-audio` entry; fix: `coppa-audio = { path=…, default-features = false, features=["file-backend"] }` in the CLI and daemon |
...
| 22 | `coppa-protocol` compiles for wasm but `session.rs` calls `std::time::Instant::now()` unconditionally → runtime panic on `wasm32-unknown-unknown` for any consumer using sessions; take a `now: Instant` parameter or a `Clock` trait (also what `no_std` needs). **`cp_negotiator.rs` needs the same treatment**: its API already accepts caller-provided `Instant`s (`apply_as_confirmer`/`tick`, `cp_negotiator.rs:425,568`) rather than calling `Instant::now()` itself, but a WASM caller of that API has no way to produce an `Instant` other than the same panicking call -- include its `now: Instant` parameters in the `Clock` abstraction too, or document CP negotiation as unavailable on wasm until that lands. | portability | M | M | `crates/coppa-protocol/src/session.rs:17,192,223,247,…`; `cp_negotiator.rs:315,425,568` (stores/requires caller-supplied `Instant`); wasm build of `coppa-protocol` `Finished` (compiles, will panic on session or CP-negotiation use) |
...
```
Caveat: `coppa-protocol` uses `std::time::Instant::now()` in non-test code in `session.rs:192` (which panics at runtime on this target for any consumer using sessions); `cp_negotiator.rs` doesn't call it outside `#[cfg(test)]` either, but its public `apply_as_confirmer`/`tick` API requires a caller-supplied `Instant`, so it hits the same unsupported-clock problem one layer up and also needs the `Clock` abstraction.
