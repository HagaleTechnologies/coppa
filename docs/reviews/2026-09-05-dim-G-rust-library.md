...
| coppa-engine | 32 (3) | 0 | 0 | 0 | `anyhow` everywhere | no (`EngineConfig`: `Debug, Clone`; no `Default`, no `serde`) | **yes** | no |
...
| 16 | Give `EngineConfig` `Default`, `PartialEq`, and `serde` derives (behind a `serde` feature); today the daemon re-implements config parsing in `coppa-daemon/src/config.rs` | api-design | M | S | `coppa-engine/src/config.rs:11` `#[derive(Debug, Clone)]` only; `CoppaCore::new()` exists but `EngineConfig::default()` doesn't (only `from_profile`) |
