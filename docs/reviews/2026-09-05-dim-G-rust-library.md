...
| 22 | `coppa-protocol` compiles for wasm but `session.rs` calls `std::time::Instant::now()` unconditionally → runtime panic on `wasm32-unknown-unknown` for any consumer using sessions; take a `now: Instant` parameter or a `Clock` trait (also what `no_std` needs). `cp_negotiator.rs` needs no such change -- its API already accepts caller-provided `Instant`s, and its only `Instant::now()` calls are `#[cfg(test)]`. | portability | M | M | `crates/coppa-protocol/src/session.rs:17,192,223,247,…`; `cp_negotiator.rs:4` (import only, unused outside `#[cfg(test)]` at line 588+); wasm build of `coppa-protocol` `Finished` (compiles, will panic on session use only) |
...
```
Caveat: `coppa-protocol` uses `std::time::Instant::now()` in non-test code in `session.rs:192` (which panics at runtime on this target for any consumer using sessions); `cp_negotiator.rs` does not call it outside `#[cfg(test)]` and needs no change.
