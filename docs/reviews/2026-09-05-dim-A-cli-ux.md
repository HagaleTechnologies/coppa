...
| 2 | `rx` on a compressed-profile frame with the default profile prints garbage hex, exit 0, no warning. Root cause: `CoppaCore::encode_bytes` prefixes compressed payloads with `COMPRESSION_MARKER` (`0xFE`), but the uncompressed path copies the raw payload verbatim (`data.to_vec()`, no escaping) -- so `0xFE` as the first payload byte is genuinely ambiguous between "compressed" and "a legitimate payload that happens to start with that byte," and a receiver cannot safely honour the marker unconditionally (an earlier draft of this item recommended exactly that, which risks feeding arbitrary binary into `lz4_decompress` and corrupting or erroring on legitimate uncompressed data). The real fix needs an unambiguous signal: a header-level compression flag (not payload-embedded), byte-stuffing/escaping of a literal `0xFE`, or a versioned envelope -- not "just trust the marker." | bug | H | M | `coppa tx "CQ CQ DE W5AU" -o std.wav --profile HF_STANDARD` then `coppa rx -i std.wav` → `Decoded: fe0a000000a007b3496692b45fc0f680`; with `--profile HF_STANDARD` → `43512043512044452057354155`; `crates/coppa-engine/src/engine.rs:172-183` (uncompressed path is `data.to_vec()`, no escaping), `:20,178,381-390` (`decompress_if_needed` gates on `compression_enabled && payload[0] == COMPRESSION_MARKER`). |
