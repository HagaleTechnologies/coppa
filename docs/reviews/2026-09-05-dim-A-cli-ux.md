...
| 2 | `rx` on a compressed-profile frame with the default profile prints garbage hex, exit 0, no warning. The wire already signals this: `CoppaCore::encode_bytes` prefixes compressed payloads with `COMPRESSION_MARKER` (`0xFE`); the receiver's `decompress_if_needed` just requires that marker *and* the local `config.compression_enabled` to both be true before decompressing, so a receiver not itself configured for compression ignores the marker it can see and prints the raw (marker + compressed bytes) hex instead. Honour the existing marker on decode regardless of local config, rather than adding a new header flag / wire-format change. | bug | H | M | `coppa tx "CQ CQ DE W5AU" -o std.wav --profile HF_STANDARD` then `coppa rx -i std.wav` → `Decoded: fe0a000000a007b3496692b45fc0f680`; with `--profile HF_STANDARD` → `43512043512044452057354155`; `crates/coppa-engine/src/engine.rs:20,178,381-390` (`decompress_if_needed` gates on `compression_enabled && payload[0] == COMPRESSION_MARKER`). |
