...
| 26 | Audio channel handling is hard-coded mono (`channels: 1`); devices exposing only stereo configs fail to open, and there is no left/right channel selection for stereo interfaces (Direwolf `ACHANNELS`/`ADEVICE` idiom). Sample rate is used verbatim with no resampler fallback -- `coppa-audio/src/resampler.rs` exists but is dormant: not declared as a `pub mod`, and its `rubato` import isn't even in the crate's dependency manifest, so it does not currently compile as part of the crate. Exposing it needs a manifest fix and validation, not just a `pub mod` line. | missing-feature | M | M | `cpal_backend.rs:126-131,152-156,222-227,248-252`; observed `Sample rate 44100 Hz is not supported`; `coppa-audio/src/lib.rs` (no `resampler` mod), `Cargo.toml` (no `rubato` dep) |
...
| 33 | Busy-channel *detection/telemetry* runs by default (`handle_audio_in` always feeds every audio block to `BusyGate` and emits `BUSY` transitions -- this is what produced the flapping observed elsewhere in this review); `busy_hold_ms = 0` only disables *courtesy TX deferral* (`wait_for_clear_channel`), not detection itself. Detection has no threshold/dB knob; gateway operators expect on-by-default deferral too, with adjustable sensitivity and a host-visible override -- but fixing this should not be framed as "turn detection on," since it already runs. | polish | M | S | `config.rs:272-283`; `coppad.toml.example:61-66`; `event_loop.rs:1589,1618` (`busy_hold_ms` gates only the deferral wait) |
