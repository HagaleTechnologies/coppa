...
| ID | P | Item | Effort | Src |
|---|---|---|---|---|
| H-090 | P0 | Two-host audio-cable loopback test (two computers, two USB sound cards, no radio): decode 20/20 golden WAVs through the cable, run in two profile-matched passes since the vectors don't share one OFDM geometry (levels 5/6/9 use `vhf_wide` for clean/AWGN vectors but `hf_standard` for Poor/SSB+CFO vectors -- a single fixed `coppad` config can't decode all 20); 10-minute ARQ session with 0 drops; repeat with ±50 ppm resample; measure RX-last-sample → PTT-on. Publish the log and WAVs under `results/ota/`. This is the cheapest possible evidence and it does not exist. | M | F#1, F protocol stage 1, E#8 |
