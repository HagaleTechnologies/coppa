...
- **On a real SSB rig the speed ladder tops out at level 4.** The engine
  ...
  10 (64-QAM 5/6) is then undecodable through the SSB filter even at 60 dB
  (LDPC non-convergence 10/10; clean unfiltered), a failure nobody had
  characterised because the SSB sweep only ever ran with VHF routing.
...
| H-002 | P0 | Stop routing speed levels ≥ 5 to `vhf_wide` on HF. Make routing follow `Profile.ofdm_profile` (currently a dead field) so every level has an HF-legal, SSB-passband profile. Levels 5–9 on `hf_standard` decode at 9/9/15/18 dB. | S | F#2, E#13, D#55 |
| H-003 | P0 | Level 10 (64-QAM 5/6) on `hf_standard` is undecodable through a 300–2700 Hz SSB filter at any SNR to 60 dB. Drop it from the HF ladder, narrow the top edge to ≤ 2600 Hz, or add edge-carrier erasure. Never characterised before this review. | S | F#2b; `results/review-2026-09-05/` |
| H-004 | P0 | VARA command port must accept bare `\r` (and `\n`, `\r\n`) as terminator; today `read_line` blocks until `\n`. | S | C#1 |
| H-005 | P0 | Forward `HostResponse::StatusUpdate` (CONNECTED/DISCONNECTED/PENDING) to the VARA command port and WebSocket; `main.rs:243` bridges only `DataOut`. No host of any kind learns session state today. | S | C#2, H#1, H#2 |
...
| ID | P | Item | Effort | Src |
|---|---|---|---|---|
| H-050 | P1 | Ship tagged releases with static binaries: linux x86-64/aarch64/armv7, Windows x86-64, macOS universal, built with `cpal-backend,websocket,serial-ptt,gpio-ptt`; `cargo binstall`; `[profile.release]` with `lto`, `codegen-units=1`, `strip`. | M | E#1, D#5, B#11, B#48, G#39 |
...
| H-059 | P2 | Audio device selection: first-substring-match with no disambiguation; hard-coded mono; no resampler fallback although `coppa-audio` ships one; no left/right channel select for stereo interfaces. | M | B#25, B#26 |
...
| H-091 | P1 | Fade-diversity / time interleaving across ≥ 1 coherence time: the "one genuinely untried lever" per ADR-006. On CCIR-Moderate everything above QPSK 1/2 sits on a 35–100 % SNR-independent outage floor; on CCIR-Poor nothing clears FER ≤ 10 %. | L | F#3; `results/review-2026-09-05/{mod,poor}-ssb-std` |
...
| H-097 | P1 | Publish an honest headline table: SSB-filtered, `hf_standard` at every level, AWGN + Good/Moderate/Poor, ARQ user bps and drop rate, side by side with IONOS VARA/PACTOR curves, with "simulation only, no OTA" stated. Resolve the SNR-reference ambiguity in BENCHMARKS.md (two contradictory BPSK tables) by relabeling every pre-3 kHz table. Add bytes/min-under-ARQ to `coppa-bench` because that is the unit every ham comparison uses. | S | F#11, E#10, E#11, E#28, D#34 |
| H-098 | P2 | Rate loop misses its own bar (0.914/0.762 vs > 1.0/≥ 0.8); active probing is built but not wired into the daemon; add an oscillation guard. | M | F#10 |
| H-099 | P2 | Dead config: `turnaround_ms`, `Profile.arq_window/max_payload/ofdm_profile` are never read; the daemon uses `ArqConfig::default()` regardless. Wire or delete. | S | F#17, B#30 |
...
| H-102 | P2 | Sync detector runs on unfiltered samples (−9 dB detection margin by design); sync, not FEC, is the dominant failure below 12 dB on Moderate/Poor. A cheap decimated pre-filter recovers it. | S | F#20; addendum tables |
...
| H-109 | P3 | `cargo bench` has no end-to-end OFDM throughput/latency benchmark (only legacy BPSK/Viterbi/FFT micro-benches). | S | F#32 |
...
| H-111 | P1 | GitHub repo metadata: description ("Open-source OFDM HF data modem for amateur radio, in Rust — VARA-style TCP API, Winlink-ready daemon, C FFI"), topics (`ham-radio`, `amateur-radio`, `hf`, `ofdm`, `ldpc`, `modem`, `winlink`, `rust`, `dsp`, `tnc`, `ax25`), homepage, social preview; enable Discussions; issue templates including a "field report" template; `CHANGELOG.md`; `docs/README.md` index. Thirty minutes of settings. | S | D#4, D#17, D#21, D#27, D#28 |
...
| H-131 | P1 | `coppa-audio`'s `default = ["cpal-backend"]` leaks into `coppa-cli`/`coppa-daemon`/`coppa-bench`, making their own `cpal-backend` features no-ops and breaking aarch64-linux `cargo check` on `alsa-sys`. Consumers should use `default-features = false`. Add `wasm32` and `aarch64-linux` `cargo check` of the library crates to CI. | S | G#12, G#56, E#3 |
| H-132 | P1 | Error strategy: replace `anyhow::Result` in public signatures of 5 of 6 library crates (`Modem` trait, `CoppaCore::decode` with string errors, 37 `pub fn` in `coppa-protocol`) with per-crate `thiserror` enums; `StreamFrame.payload: Result<Vec<u8>>` embeds an error in an event struct. | L | G#3, G#29, G#44 |
...
| H-136 | P2 | Portability: `coppa-protocol` compiles for wasm but calls `Instant::now()` unconditionally in `session.rs`/`cp_negotiator.rs` (runtime panic on `wasm32-unknown-unknown`); take a `now` parameter or a `Clock` trait (also the `no_std` path). `coppa-channel` fails on wasm via `getrandom`; take an `Rng` parameter. | M | G#22, G#23, G#42 |
...
| H-152 | P2 | Station ID: an idle connected session never IDs; a disconnect frame should carry ID; CWID option (H-038). | S | F#34 |
...
H-003, H-004–H-009, H-014, H-015, H-018–H-024, H-070, H-090 (cable test),
