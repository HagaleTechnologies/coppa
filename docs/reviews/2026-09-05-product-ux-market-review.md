...
|---|---|---|---|---|
| H-001 | P0 | TX truncation, both daemon and CLI: `handle_audio_out`/`CpalSink::write` are drop-on-full into an 8,192-sample ring; a 65,520-sample frame loses 87 %. This is not daemon-only -- `coppa-cli/src/main.rs:377-408`'s live `coppa tx`/`coppa tune` build the same fixed-8,192-sample `CpalSink`, call `sink.write(samples)` once, and discard the returned partial-write count via `?`, so live CLI transmission also silently sends only the first 8,192 samples of a normal frame while reporting success. Chunk/pace the write at the sample rate (or size the ring >= max frame + guard) in both the daemon and the CLI, and derive PTT release from samples actually delivered. **Ring enlargement alone is not sufficient and must be paired with TX serialization**: established-session data-port reads call `transmit_samples` directly (`event_loop.rs:830-836`), bypassing `tx_queue` and its `is_transmitting` guard, and each call spawns an independent, un-cancelled PTT-release timer (`event_loop.rs:1578-1584`). A merely-larger ring lets a second frame queue while the first is still playing, and the first frame's timer can then deassert PTT mid-transmission of the second. Route every TX source through one serialized queue, or otherwise ensure a single extendable PTT-release deadline, in addition to whichever ring-sizing fix is chosen. | M | B#1, H#9, C#48; CLI path new finding, `coppa-cli/src/main.rs:377-408` |
...
| H-003 | P0 | Level 10 (64-QAM 5/6) on `hf_standard` fails to clear FER <= 10% at every tested SNR point through a 300–2700 Hz SSB filter (0-27 dB step 3, and 30/40/50/60 dB -- not a continuous sweep, 90-100% FER, LDPC non-convergence). Drop it from the HF ladder, narrow the top edge to ≤ 2600 Hz, or add edge-carrier erasure. Never characterised before this review. | S | F#2b; `results/review-2026-09-05/` |
...
| ID | P | Item | Effort | Src |
|---|---|---|---|---|
| H-070 | P1 | Silent exit-0 failures: wrong-sample-rate WAV, zero frames decoded, compressed-profile frame decoded with the default profile (prints garbage hex), unknown `--ptt` value, unmatched `--device` (falls back to the laptop speaker), rigctld connect failure (plays audio unkeyed) -- and **also silent failures *after* a successful rigctld connection, in both the CLI and the daemon**: `transmit_live` discards the result of both `set_ptt(PttState::Tx)` and `set_ptt(PttState::Rx)` (`coppa-cli/src/main.rs:401,411`), so an `RPRT` error or a dropped socket at either call either transmits without keying the radio or leaves it keyed after TX, while the CLI reports success either way -- and `coppad` has the identical gap in every mode: `EventLoop::handle_ptt_change` logs and discards `self.ptt.set_ptt`'s error, then reports telemetry and updates `is_transmitting` as though the hardware actually changed state (`event_loop.rs:2713-2727`), and TNC mode discards both PTT results the same way (`tnc.rs:225,229`). A daemon TX-assertion failure therefore sends audio without keying, and an RX-deassertion failure can leave the rig keyed indefinitely with no visible error in any mode. Most of these should simply become an error, but the compressed-frame case can't: the wire format has no header-level compression bit (`COMPRESSION_MARKER = 0xFE` prefixes compressed payloads, `crates/coppa-engine/src/engine.rs:20`; uncompressed payloads copy arbitrary bytes unchanged), so an uncompressed payload that happens to start with `0xFE` is indistinguishable on the wire from a compressed one — there is no way to detect "wrong profile" and error on it without first adding the flag/escaping/versioned-envelope fix A#2 already calls for. For the PTT command failures specifically: propagate the TX-assert error as fatal and make an RX-deassert failure a visible fatal error with a failsafe unkey path, in the CLI, the daemon's runtime path, and TNC mode alike -- not only in the shutdown/fatal-exit cleanup paths that already handle this correctly. | S | A#2–4, A#9, A#12, A#13 |
...
| ID | P | Item | Effort | Src |
|---|---|---|---|---|
| H-090 | P0 | Two-host audio-cable loopback test (two computers, two USB sound cards, no radio): decode golden WAVs through the cable, run in two profile-matched passes since the vectors don't share one OFDM geometry (levels 5/6/9 use `vhf_wide` for clean/AWGN vectors but `hf_standard` for Poor/SSB+CFO vectors -- a single fixed `coppad` config can't decode all 20). Realistic target is **18/20, not 20/20**: `L1_awgn12`/`L2_awgn12` decode fine via the batch API but produce zero frames via the daemon's actual `StreamingReceiver::push_samples` path (`rx_golden.rs`'s own documented, pre-existing exclusion) -- root cause is H-102's unfiltered-sync-detector margin, deferred to Phase 2, so don't gate Phase 1 on those two vectors. 10-minute ARQ session with 0 drops; repeat with ±50 ppm resample; measure turnaround against **both** a maximum latency bound (RX-last-sample → this station's PTT-on, target ≤ 250 ms) **and** a minimum-turnaround guard against the peer's actual PTT-off edge (last audio sample + `drain_ms` + `tail_delay_ms`, via a scope or GPIO tap on both stations, per §7 item 1 below) — a ceiling-only check would let a broken H-104 (zero-turnaround ACK) pass this gate. Publish the log and WAVs under `results/ota/`. This is the cheapest possible evidence and it does not exist. | M | F#1, F protocol stage 1, E#8; streaming gap per `crates/coppa-cli/tests/rx_golden.rs` |
...
| H-104 | P1 | **There is no RX-to-TX turnaround delay -- the ACK can be transmitted while the peer is still keyed and unable to receive it.** The receive arm calls `transmit_samples` for the ACK immediately once `ack_info` is computed (`event_loop.rs:1330-1339`); no production code reads `turnaround_ms` at all. Meanwhile the sending peer's own PTT-release timer (`event_loop.rs:1561-1584`) keeps it keyed for `audio_duration_ms + drain_ms + tail_delay_ms` after its last sample -- on a real half-duplex rig, an ACK sent that fast will frequently go out while the other station is still transmitting and deaf to it. This is a correctness defect on real RF, not just an unmeasured latency, and it survives even after H-001's TX-truncation fix. Implement and verify an actual RX->TX turnaround delay before any two-radio test. | M | F#16 |
...
| H-124 | P2 | FFI: `cpp_compat = true` (`extern "C"` guard), `COPPA_API` export macro, `coppa_last_error()`/`coppa_strerror()`, distinguish "no message" from error, document thread safety (handles are internally locked), a `sample_rate()` accessor **preceded by real sample-rate validation** (see below), `max_payload()` accessor, `struct_size` on `coppa_config_t` before v2 is a stable ABI, `deprecated` attributes on the v1 quartet, CI job compiling a C smoke test and publishing header + cdylib per platform, `coppa.pc`. H-023 flags the daemon-config side of this gap; it extends to FFI too — `coppa_engine_new_with` copies any nonzero `cfg.sample_rate` straight into `EngineConfig` (`coppa-ffi/src/lib.rs:406-408`) but `CoppaCore::build` constructs a fixed-48 kHz OFDM profile without ever consulting that field (`coppa-engine/src/engine.rs:134-138`), so a caller requesting e.g. 44.1 kHz gets a valid handle that still emits/expects 48 kHz. Add `0 or 48000` validation at the FFI boundary (or implement real resampling) before adding the accessor, so it can't report a configured-but-unhonoured value. | M | C#39–43, G#6 |
...
| H-131 | P1 | `coppa-audio`'s `default = ["cpal-backend"]` leaks into `coppa-cli`/`coppa-daemon` (verified: `coppa-bench` has no `coppa-audio` dependency at all, so it is unaffected -- an earlier draft of this item wrongly included it), pulling in cpal/alsa dependencies even when their own local `cpal-backend` feature is off (their runtime audio-setup code still correctly gates on that local feature, so this is unwanted dependency activation, not a functional no-op) -- and it breaks aarch64-linux `cargo check` on `alsa-sys`. The CLI and daemon should use `default-features = false`. Add `wasm32` and `aarch64-linux` `cargo check` of the library crates to CI. | S | G#12, G#56, E#3 |
...
| H-136 | P2 | Portability: `coppa-protocol` compiles for wasm but `Session`'s API (`session.rs`) calls `Instant::now()` unconditionally (runtime panic on `wasm32-unknown-unknown` for any consumer using sessions); take a `now` parameter or a `Clock` trait (also the `no_std` path). **Correction: `cp_negotiator.rs` needs the same refactor, not none.** It's true no production code in this module *calls* `Instant::now()` -- but its state stores an `Instant` (`probation: Option<(Instant, CpMode)>`, `cp_negotiator.rs:315`) and its public `apply_as_confirmer`/`tick` methods (`cp_negotiator.rs:425,568`) both *require the caller to supply one*. A WASM caller has no source for that `Instant` other than the same panicking `Instant::now()`, so CP negotiation hits the identical unsupported-clock wall one layer up, before this module's own code runs. Include `cp_negotiator.rs`'s `now: Instant` parameters in the proposed `Clock`/elapsed-time abstraction alongside `session.rs`, or explicitly document CP negotiation as unavailable on `wasm32-unknown-unknown` until that lands. `coppa-channel` fails on wasm via `getrandom`; take an `Rng` parameter. | M | G#22, G#23, G#42 |
...
| H-141 | P1 | Web dashboard served by `coppad` (ardopcf pattern: one static HTML file, vanilla JS + canvas waterfall, embedded with `include_str!`): waterfall/spectrum, RX level meter, S/N sparkline, our and peer speed level, CFO, PTT/BUSY lamps, buffer, session panel (state, peer, in-flight/retries/RTO), heard list, TUNE buttons, log pane, MYCALL/connect/send box. Prerequisite the H-120/H-121 API work doesn't cover: `WsClientMessage` has no `tune` or `set_speed_level` variant today, so the dashboard's TUNE buttons and level-selection control have no daemon request path to call -- add those request messages and their `event_loop` handlers alongside H-120's push-event work, not just telemetry. Every reference modem has this; without it an operator cannot set a level or tell TX from a hang. Wireframe in dimension H. No native GUI, no Electron. | M | H#11–14, H#18, E#17; missing WsClientMessage variants new finding |
...
| ID | P | Item | Effort | Src |
|---|---|---|---|---|
| H-150 | P0 | Gate `vhf_wide` (350–5900 Hz) off below 29.7 MHz in **every live-transmit frontend, not just `coppad`**: 47 CFR 97.307(f)(3) caps HF data at 2.8 kHz since 2024-01-08. H-002 alone does not satisfy this -- it fixes the *configured* profile to persist, but neither the daemon nor the CLI has a frequency source to gate against; see H-027 for the actual frequency-awareness gap this bullet depends on. Scoping the fix to `coppad` alone is insufficient: `coppa tx --profile VHF_FAST --ptt rigctld ...` resolves to speed level 9/`vhf_wide` through the CLI's own path, and `build_ptt` (`coppa-cli/src/main.rs:324`) only constructs a type-erased `Box<dyn PttControl>` for keying -- it never queries or validates the tuned frequency even over `rigctld`, which could report it. Apply the band/frequency gate (or fail closed when frequency is unavailable) to both the daemon and the CLI's live-transmit path. | S | E#13; H-027 |
...
| H-152 | P1 | **Station ID has a real gap, not just a design choice -- and the fix needs to be architectural, not per-call-site.** `transmit_samples` correctly prepends the ID/beacon whenever `id_due()` is true, and a completely silent session (never transmits) correctly never IDs -- that part is by design and fine. But `last_id_time` initializes to `Instant::now()` at daemon startup and `id_due()` stays false until the full interval elapses (`event_loop.rs:180,358,1644-1658`), so a real transmission inside that window goes out with no ID at all. This review tried patching that per call site across three rounds -- force an ID on the first transmission; then on session-end `DISCONNECT` too, once it was clear the first-TX fix alone left the *last* transmission of a session unidentified; then it became clear a standalone raw/ARQ send or `TUNE` with no `DISCONNECT` still has no closing ID either, because `transmit_samples` only ever *prepends* a beacon (`event_loop.rs:1532-1545`) and never appends one -- each fix was locally correct but the next call site was always missed, because Part 97.119 actually asks for two independent things this design conflates: a periodic reminder ID during a long communication (what `id_due()`'s interval timer already does, and does correctly), and a guaranteed closing ID at the end of *every* communication, standalone or session-based. Enumerating call sites (first-TX, DISCONNECT, raw/TUNE, and whatever the next TX path turns out to be) will keep finding new gaps. Fix: give `transmit_samples` (or a thin wrapper every TX call site already routes through, per this file's own "none of them are exempted" invariant) an unconditional closing-ID step -- append an ID postscript to the last frame of any transmission that is not immediately followed by another queued frame in the same communication, independent of `id_due()`'s interval timer -- so every current and future call site inherits correct trailing identification automatically instead of needing its own explicit fix. Keep the existing `id_due()`-gated prepend for the periodic in-communication reminder; it is not what is broken here. Remaining work after that: a CWID option (H-038). | S | F#34 |
...
H-003, H-004–H-009, H-014, H-015, H-018–H-027, H-030, H-034, H-062 (WebSocket auth), H-070, H-090 (cable test), H-104 (turnaround delay -- a real-RF correctness bug, not a Phase-2 measurement task), H-152 (closing-ID station identification, architectural not first-TX-only -- reclassified P1, a compliance gap),
