...
| ARQ | Selective-repeat + IR-HARQ (NR BG2) implemented and real for the unconnected/raw data path and CP-negotiation control PDUs -- but the user-facing **connected session bypasses it entirely** (H-025: established-session TX sends a MacPdu directly, RX forwards without ArqTx/ArqRx), so a Pat/Winlink-style session has no retransmission today. The cited session bench (drop-free on only 2/5 Good, 0/5 Mod/Poor) exercises the standalone ARQ model, not a daemon connected session, so even those numbers are optimistic versus what a real session gets. [repo: dim-F report, COP-3 re-run; H-025] | Yes, adaptive; never dropped in IONOS | Yes; frequent drops on MPP | Yes | HARQ + Chase combining, OLLA | Yes, memory-ARQ |
