...
| Max throughput | **Sim only**: peak goodput 9,976 bps (64QAM 7/8, AWGN, pre-rebaseline) / 6,852 bps 16QAM 3/4 (2026-07 re-baseline, 3 kHz-ref) [repo: BENCHMARKS.md]; **no OTA number** | 8,489 bps L17 [vendor]; ≈46 kB/min at 30 dB WGN [IONOS measured] | ≈10 kB/min (≈1.3 kbps) at 30 dB WGN [IONOS] | ≈980 bps (datac1) raw | 3,100 bps raw (QAM16C2); ≈142 B/s goodput under stop-and-wait ARQ [self-published] | ≈40 kB/min (≈5.5 kbps) [IONOS]; 10.5 kbps spec |
| Min-SNR sensitivity | BPSK 1/2 FER≤10% at **12 dB AWGN, 21 dB Watterson-Moderate, never on Poor** (2026-07, 3 kHz-ref) [repo: BENCHMARKS.md]; MIL-STD ladder passes 0/27 | ≈2,000 B/min still flowing at 5 dB WGN; L1 at 18 bps designed for negative SNR [IONOS/vendor] | ARDOP 500 works to ~5 dB WGN but drops on MPP [IONOS] | datac4 90/100 at −4 dB MPP, datac3 74/100 at 0 dB MPP [codec2 measured] | DATAC16 −9.3 dB AWGN / −4 dB MPP; DATAC17 +10 dB MPP [self-published] | P4 ≈2,000 B/min at 0 dB WGN; never drops [IONOS] |
| ARQ | Selective-repeat + IR-HARQ (NR BG2), multi-codeword frames; session bench drop-free on only 3/5 Good, 0/5 Mod/Poor [repo: CLAUDE.md] | Yes, adaptive; never dropped in IONOS | Yes; frequent drops on MPP | Yes | HARQ + Chase combining, OLLA | Yes, memory-ARQ |
| Compression | Huffman + LZ4 [repo] | Yes (proprietary) | Host-side (Winlink LZHUF) | Yes | COMPRESSION cmd is a no-op | Yes (PMC) |
| Host API | VARA-style TCP 8300/8301 subset (MYCALL, LISTEN, CONNECT, DISCONNECT, ABORT, BW500/2300/2750, COMPRESSION, PTT, VERSION, BUSY, KISS) [repo: coppa-host]; **no CWID, PUBLIC, CQFRAME, SN, BITRATE, BUFFER, TUNE-on-TCP verified**; WebSocket JSON; KISS TCP for AFSK; C FFI | VARA TCP (de-facto standard) | ARDOP host protocol TCP 8515 | REST + WebSocket | VARA-style TCP + KISS-over-TCP | Serial/USB AT-style |
