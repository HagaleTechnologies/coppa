...
with a VARA-style TCP interface, 9 speed levels, NR-BG2 LDPC (IR-HARQ not yet wired into retransmits -- see ARQ row below), AFSK/KISS, C FFI and WebSocket API — but
...
- **Alignment check for coppa** [repo: `docs/SPEC.md` §1.2, §11]: `hf_standard`/`hf_robust` occupy 350–2700 Hz
  ...
  Station-ID is present but the periodic timer alone is incomplete, not just missing CWID: `coppad.toml.example`
  documents `[station_id] id_interval_secs = 540` (9 min, under the 10-min FCC margin), `StationIdConfig` defaults
  to the same value, and `transmit_samples` (the single TX chokepoint) verifiably prepends the ID/beacon whenever
  due -- but `last_id_time` suppresses that prepend for the first ~9 minutes after daemon startup, and
  `transmit_samples` only ever *prepends* a beacon, never appends one, so a short session or a standalone raw/TUNE
  transmission can still complete with zero identification (see H-152, now reclassified P1 for exactly this gap,
  not just first-transmission timing). What is genuinely missing on top of that closing-ID fix is CWID (a Morse
...
| Bandwidth modes | Defined in SPEC but only two are actually reachable in code today: 2400 Hz (`hf_standard`, used by every named "hf" profile including `HF_ROBUST`/`EMERGENCY` -- `CoppaCore::from_profile` collapses any non-"vhf" profile to `hf_standard()` regardless of name) and 5.5-5.9 kHz VHF (`vhf_wide`, levels >= 5). `hf_narrow` (500 Hz) and `hf_wide` (2450 Hz) exist as constructors in `profiles.rs`/SPEC §1.1 but no operational path selects them -- they are dormant, not product modes [repo: `engine.rs:119-125`] | 500 / 2300 / 2750 Hz | 200 / 500 / 1000 / 2000 Hz | 250–1700 Hz codec2 modes; 500/2438 Hz OFDM WIP | 200 / 250 / 500 / 1700 / 2100 Hz | 500 Hz (P1–2), 2400 Hz (P3/4) |
| Max throughput | **Sim only**: peak goodput 8,450 bps (64QAM 5/6, `vhf_wide`, AWGN, from the older default-routing dataset) [repo: `results/p2-final/awgn.csv`]; **no OTA number**, and this figure requires the VHF-bandwidth profile and default (not `hf_standard`-forced) routing -- this review's own fresh `hf_standard`-forced re-sweep (`results/review-2026-09-05/`) never exercised `vhf_wide`, and the SSB-realistic HF ladder tops out far lower (see dim-F) | 8,489 bps L17 [vendor]; ≈46 kB/min at 30 dB WGN [IONOS measured] | ≈10 kB/min (≈1.3 kbps) at 30 dB WGN [IONOS] | ≈980 bps (datac1) raw | 3,100 bps raw (QAM16C2); ≈142 B/s goodput under stop-and-wait ARQ [self-published] | ≈40 kB/min (≈5.5 kbps) [IONOS]; 10.5 kbps spec |
| Min-SNR sensitivity | BPSK 1/4 (level 1) FER≤10% at **6 dB AWGN** (this review's fresh re-sweep), **12 dB Watterson-Good** (older 400-trial committed dataset -- not rerun in this review), **24 dB Watterson-Moderate** (this review's fresh 50-trial sweep; the older 400-trial dataset showed 12 dB, but a Wilson bound at 50 trials is wider), **never clears Watterson-Poor with a Wilson bound** (fresh 50-trial best point is 30 dB at 8% raw FER / 18.8% upper bound; the older 400-trial dataset's best point, also 30 dB, is 9.5% raw / 12.8% upper bound -- neither clears 10%) [repo: `results/review-2026-09-05/` for AWGN/Moderate/Poor, `results/p2-final/` for Good]; MIL-STD ladder passes 0/27 | ≈2,000 B/min still flowing at 5 dB WGN; L1 at 18 bps designed for negative SNR [IONOS/vendor] | ARDOP 500 works to ~5 dB WGN but drops on MPP [IONOS] | datac4 90/100 at −4 dB MPP, datac3 74/100 at 0 dB MPP [codec2 measured] | DATAC16 −9.3 dB AWGN / −4 dB MPP; DATAC17 +10 dB MPP [self-published] | P4 ≈2,000 B/min at 0 dB WGN; never drops [IONOS] |
| ARQ | Selective-repeat implemented and real for the unconnected/raw data path and CP-negotiation control PDUs -- but the user-facing **connected session bypasses it entirely** (H-025: established-session TX sends a MacPdu directly, RX forwards without ArqTx/ArqRx), so a Pat/Winlink-style session has no retransmission today. **IR-HARQ (NR BG2) itself is not wired into the raw path either**, distinct from H-025's connected-session gap: `CoppaCore::encode_bytes` hard-codes `fec_type: 0` and `seq_num: 0` in every PHY header (`engine.rs:193-203`) regardless of retransmit attempt, and the daemon's retransmit driver (`check_arq_retransmits`) never calls `rv_for_attempt` to select RV1/RV2/RV3 -- so retries always resend the identical RV0 encoding, and a failed decode's persistent PHY accumulator for sequence 0 can even combine LLRs across unrelated retransmitted segments. The cited session bench (drop-free on only 2/5 Good, 0/5 Mod/Poor) exercises the standalone ARQ model, not a daemon connected session, so even those numbers are optimistic versus what a real session gets, and don't reflect IR-HARQ gains that aren't actually happening. [repo: dim-F report, COP-3 re-run; H-025] | Yes, adaptive; never dropped in IONOS | Yes; frequent drops on MPP | Yes | HARQ + Chase combining, OLLA | Yes, memory-ARQ |
...
| 10 | The `session` bench already reports bytes/min-under-ARQ (net of retries) -- the missing work is running it at the IONOS study's fixed WGN/MPG/MPP SNR points (not an ad-hoc ramp) and publishing directly-comparable curves with CIs, in IONOS format | evidence-gap | H | S | Every ham comparison uses B/min (IONOS; Mercury MODES.md goodput); `session` bench already produces bytes/min, see this report's evidence-gap note |
...
| 12 | Rewrite README status table to match SPEC (OFDM working, 9 levels [1-7,9,10; level 8 reserved], LDPC NR BG2, VARA-TCP, KISS/AFSK, WebSocket); **don't claim IR-HARQ** -- it's not wired into retransmits (see ARQ row in the table above); add a 30-second "what it is / isn't" | positioning | H | S | README says OFDM "Partial", QPSK+ "not wired"; `SPEED_LEVELS`/`VALID_SPEED_LEVELS` both have 9 entries |
