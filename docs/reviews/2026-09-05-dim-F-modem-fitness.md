...
| 34 | **Station ID is only prepended to a real transmission** — verified: `transmit_samples` (the single TX chokepoint, `event_loop.rs:1529-1545`) already prepends the ID/beacon whenever `id_due()` is true, so a DISCONNECT frame (which goes through the same chokepoint) already carries ID if the interval has elapsed. A completely silent session that never transmits never IDs, which is correct per Part 97.119 (ID required at end of communication and every 10 min *while transmitting*). No action needed here. | safety | - | - | `event_loop.rs:1003-1010,1529-1545` |
