...
1. **The high-speed half of the ladder is not an SSB waveform.** `CoppaCore::select_ofdm_profile` (`crates/coppa-engine/src/engine.rs:150`) routes every speed level ≥ 5 to `vhf_wide` (350–5900 Hz, 60-sample CP). Through a 300–2700 Hz SSB filter, levels 5–10 decode **zero frames at any SNR** (`results/p2-final-ssb/awgn.csv`), and even unfiltered the 60-sample CP is shorter than the CCIR-Poor 2 ms tap. So the headline "8.4 kbps at level 10" is a VHF-bandwidth number; on a real HF SSB rig the daemon's ladder tops out at **level 4, ≈1.8 kbps PHY / ≈0.48 kbps ARQ user at the daemon's shipped one-codeword-per-frame, one-ACK-per-frame default** (the idealized win-8/150 ms figure is 1.4 kbps, and even the batched-ACK design modelled below is 0.95 kbps -- neither is what the daemon actually sends today, which ACKs every single data frame). Forcing `hf_standard` for the whole ladder (this review's sweep) recovers levels 5–9 (up to 3.2 kbps PHY at 18 dB) but **level 10 then fails to clear FER ≤ 10 % at any SNR through a 300–2700 Hz SSB filter to 60 dB** (90–100 % FER, LDPC non-convergence; clean without the filter) — a never-before-characterised hard failure. The `hf_robust`/`hf_narrow`/`hf_wide` profiles in SPEC §1.1 are unreachable from `select_ofdm_profile`'s speed-level routing (`engine.rs:150`), although `Profile.ofdm_profile` itself IS read by `CoppaCore::from_profile` at initial construction — the gap is that a later `set_speed_level` reconfiguration goes through `select_ofdm_profile`, which recomputes the HF/VHF choice from the level alone and ignores the profile's `ofdm_profile` field entirely.
2. **Fading robustness is far below VARA/PACTOR class.** On Watterson CCIR-Moderate (1 ms/0.5 Hz), this review's own fresh 50-trial re-sweep (the more current run) has level 1 first clearing FER <= 10 % at 24 dB and level 2 never clearing through 30 dB -- worse than the older 400-trial dataset's 12/15 dB, but that difference is at least partly a sample-size artifact (a Wilson bound is wider at 50 trials than 400 for a similar underlying FER), not necessarily a real regression; a larger-trial re-run at the fresh SSB/`hf_standard` configuration would resolve which figure is right. Either way, on CCIR-Poor (2 ms/1 Hz) nothing clears FER ≤ 10 % with a Wilson bound at any tested SNR up to 30 dB in either dataset (level 1's best raw FER is 8-9.5 % depending on trial count, but the upper bound stays above 10 %), level 9 never clears the FER <= 10% target under any fading through 36 dB (it does produce some successful decodes even at a high outage rate -- e.g. 32% success/68% FER on Good at 30 dB, 15%/85% on Moderate, 3.5%/96.5% on Poor -- "never converges" overstated this as total failure rather than a high but non-total outage floor), and the simulated 10-minute ARQ session bench **survives 0/5 Moderate and 0/5 Poor sessions (5/5 drop) and only 2-3/5 Good sessions** — the exact failure mode the 2020 Winlink IONOS study identified as what separates ARDOP-class from VARA/PACTOR-class modems. Net session goodput in that bench is 390–530 bytes/min on Poor and 1,100–1,700 bytes/min on Good, versus VARA-2300's ≈3,500 (MPP) / ≈3,000 (MPG) bytes/min at the same ≈10 dB mid-ramp SNR in the IONOS study.
...
| Throughput bench | `benches/throughput.rs` (criterion: `engine_encode`/`engine_decode` on `CoppaCore` -- real end-to-end OFDM benches at the default level -- plus AGC, RRC, FFT, Viterbi, BPSK primitive micro-benches) | not run | `engine_encode`/`engine_decode` ARE end-to-end OFDM benches, but only at one (default) speed level; there is no multi-level sweep, no streaming/daemon-path latency bench, and no OTA goodput bench. Load average made timing meaningless anyway. |
...
|---|---|---|---|---|---|---|---|---|
| 1 | BPSK 1/4 | 6 dB (24 % FER @ 3) | 6 | 6 | 12 | 12 | never (30 dB best: 9.5 % raw, fer_hi 12.8 %, misses Wilson) | 352 |
| 2 | BPSK 1/2 | 6 | 6 | 6 | 12 | 15 | never | 709 |
...
Frame = 3 preamble/probe + 4 header + ⌈⌈1944/bps⌉/44⌉·cw payload symbols × 26.25 ms; payload = (k_used/8 − 4)·8·cw bits. ARQ cycle = 8 data frames + 2 turnarounds + 1 ACK frame (ACKs are full frames encoded at the *current* speed level via `engine.encode_bytes`; modelled here at L2 = 1.365 s, the worst realistic case). **The last column is a hypothetical batched-ACK design, NOT what is shipped**: `max_frames_per_turn = 4` is set but never read outside a tracing field (`event_loop.rs:840`), and the daemon actually sends one full ACK frame per received data PDU (`event_loop.rs:1322-1339`, explicit "one ACK per frame" per ADR-007/008 decision 4) with no batching at all. A corrected per-frame-ACK model for the shipped daemon (same `ptt_pre 50 ms + ptt_tail 200 ms + turnaround 150 ms` per switch, 2 switches per data-frame/ACK round trip = 800 ms overhead, ACK still modelled at L2 = 1.365 s) gives level 4 (the level that propagated into the headline and comparisons below) **≈ 482 bps**, not 947 bps — roughly half. This recompute is this reviewer's own approximation, not verified against a captured daemon trace; treat the whole "daemon defaults" column below as illustrative of the *batched* design intent only, and use ≈482 bps (or a comparably-derived per-frame-ACK figure for other levels) wherever a shipped-daemon throughput number is needed.

| Level | Frame airtime (1 cw / 8 cw) | PHY goodput 1 cw / 8 cw (bps) | Spectral eff. (b/s/Hz in 2.35 kHz) | ARQ user, win 8, 150 ms (bps) | ARQ user, hypothetical batched design, 4 frames/turn (bps) -- NOT shipped |
|---|---|---|---|---|---|
...
| Condition | coppa (SSB-realistic: L ≤ 4 `hf_standard`, shipped daemon per-frame-ACK ceiling) | coppa (if L5–10 were on `hf_standard`, 8 cw, idealized) | VARA HF 2300 | PACTOR-4 (2.4 kHz) | ARDOP 2000 |
|---|---|---|---|---|---|
| AWGN 30 dB | ≈0.48 kbps (L4) | ≈6.1 kbps (L10, AWGN only) | ≈6.1 kbps (46,000 B/min) | ≈5.3 kbps (40,000) | ≈1.3 kbps (10,000) |
| AWGN 20 dB | ≈0.48 kbps (L4) | ≈3.9 kbps (L7) | ≈3.1 kbps (23,500) | ≈5.3 kbps (40,000) | ≈0.7 kbps (5,500) |
| AWGN 10 dB | ≈0.48 kbps (L4, clears at 6 dB) | same | ≈0.67 kbps (5,000) | ≈2.1 kbps (15,500) | ≈0.2 kbps |
| AWGN 5 dB | ≈0.1 kbps (L1 @ 24 % FER, per-frame-ACK model) | — | ≈0.27 kbps (2,000) | ≈0.53 kbps (4,000) | ≈0.07 kbps |
...
| CCIR Poor 20 dB | L4 989 bps PHY @ ≈45 % FER → *measured* ARQ session ≈0.05–0.07 kbps; **links drop** | L3–4 never clear FER 10 % | ≈1.5 kbps (11,000), no drops | ≈2.9 kbps (21,500), no drops | ≈0.13 kbps, drops |
| CCIR Poor 10 dB | ≈0 (L1 FER > 60 %) | — | ≈0.47 kbps (3,500) | ≈1.3 kbps (10,000) | ~0 |
| CCIR Good 20 dB | L2 (12 dB clear) ≈0.27 kbps per-frame-ACK ceiling; *measured* ARQ session ≈0.15–0.23 kbps | L3+ never clear FER 10 % (milstd) | ≈1.6 kbps (12,000) | ≈2.9 kbps (21,500) | ≈0.2 kbps |
...
AWGN rows use the corrected per-frame-ACK ceiling derived earlier (one full ACK per data frame, the daemon's shipped behaviour); the idealized batched-design/window-8 figures used in an earlier draft of this table were roughly 2-3x too high and have been replaced. CCIR Poor/Good rows marked *measured* come from the actual simulated 10-minute ARQ session bench, not this analytic model, and are unaffected by the correction.
...
**Reading:** with the corrected per-frame-ACK ceiling, coppa's shipped L4 (≈0.48 kbps) trails VARA even in the AWGN mid-range where it looked competitive before this correction (VARA ≈0.67 kbps at 10 dB); only the *idealized, non-shipped* batched-ACK design (≈1.4 kbps, H-094/block-ACK) would approach parity, and only if that work is actually done. With L5-10 restored to an SSB profile plus multi-codeword frames, coppa's 20-30 dB AWGN *PHY* ceiling would still match VARA's -- but that too needs H-002 and H-094 shipped, not just simulated. The problems remain the two ends and the middle that matters: nothing below +4 dB, nothing survives CCIR-Poor, and the link drops under fading. Spectral efficiency at the top (1.7 b/s/Hz PHY at L10 `hf_standard`) is fine; the missing ingredients are fading diversity and a real ACK-batching implementation, not constellation size.
...
Follow-up (10 trials/pt): level 10 on `hf_standard` **without** the SSB filter decodes 0/10 errors at 30, 45 and 60 dB (peak 4023 bps); **with** the filter it fails 9/10 at 30 dB and 10/10 at 40, 50 and 60 dB, always `ldpc_not_converged`, never sync/header. So level 10 fails to clear FER ≤ 10% at any tested SNR through a 300–2700 Hz SSB passband on the HF profile (90–100% FER, LDPC non-convergence) — a hard failure, not a late waterfall. Likely mechanism (not verified): `hf_standard`'s top carriers sit at 2650–2700 Hz, inside the filter's transition band; 64-QAM rate-5/6 has no margin for a few attenuated edge carriers. Whatever the cause, the shipped ladder's top HF-capable level on a real rig is **level 9 at 18 dB / 3.2 kbps PHY (1 codeword)**, and levels 5–9 only if item 2 (profile routing) is fixed. Note the per-level goodputs match the analytic `hf_standard` table above exactly.
...
- **Test discipline**: 1,137 `#[test]`/`#[tokio::test]` functions, proptest round-trips (2 regression seeds saved, both trivial `[254]`), loss-injection daemon↔daemon tests for every droppable CP-negotiation frame, regression tests that were verified to fail pre-fix, full-suite CI ≈14.5 min.
...
| 2 | **Stop routing levels ≥ 5 to `vhf_wide` for HF.** Give the ladder an HF profile at every level, or make `set_speed_level`'s routing honour `Profile.ofdm_profile` (it IS read at initial construction via `CoppaCore::from_profile`, but `select_ofdm_profile` recomputes HF/VHF from the level alone on a later speed-level change, ignoring it). Levels 5–10 decode 0 frames through an SSB filter; forced onto `hf_standard` levels 5–9 work at 9/9/15/18 dB (this review's sweep). | robustness | H | S | `engine.rs:150-159`; `results/p2-final-ssb/awgn.csv` L5–10 all-fail; milstd/session had to force `hf_standard`; fresh sweep above |
| 2b | **Level 10 (64-QAM 5/6) fails to clear FER ≤ 10 % at any SNR through a 300–2700 Hz SSB passband on `hf_standard` up to 60 dB** (90 % FER at 30 dB, 100 % at 40–60 dB, all `ldpc_not_converged`), while decoding cleanly unfiltered. Either drop level 10 from the HF ladder, narrow `hf_standard`'s top edge (2700 → ≤ 2600 Hz), or add edge-carrier erasure handling. Never characterised before because the SSB sweep only ever ran with default (VHF) routing. | robustness | H | S | `results/review-2026-09-05/awgn-ssb-std{,-hi}/awgn.csv` (this review); `results/p2-final-ssb` never exercised `hf_standard` at L10 |
| 3 | **Fade-diversity across a coherence time.** Correction to the ADR-006 quote cited here (now superseded): an eight-frame V2 cross-frame interleaver WAS implemented and measured (`BENCHMARKS.md` "Transfer-level cross-frame interleaving"), and it made Watterson recovery worse, not better -- the design is shelved, falsified, not untried. L9 perfect-CSI bound admits 91.6 % of Good frames at 30 dB vs 15 % real; every level ≥ 3 fails FER 10 % on Good/Moderate; no within-frame interleaver can save a 1.365 s BPSK frame from a 1 Hz fade. Any further diversity work needs a materially different mechanism (frequency diversity, or a longer codeword spanning multiple fades), named explicitly. | perf | H | L | COP-4 section; ADR-006 "Fade-diversity interleaving is now the one genuinely untried lever" (superseded); `BENCHMARKS.md` "Transfer-level cross-frame interleaving"; `results/p2-final/moderate.csv` |
...
| 10 | **Rate loop still misses its bar (0.914 / 0.762 vs > 1.0 / ≥ 0.8).** Active probing IS wired into the daemon (`event_loop.rs:289-293,569-587`). The project's own committed diagnostics falsified an oscillation/hysteresis-shape fix and traced the real remaining lever to the per-frame recommendation signal's volatility during fast fading, not hysteresis tuning -- fix that signal instead. | perf | M | M | CLAUDE.md RateLoop diagnostic; `event_loop.rs:289-293,569-587`; `closed_loop_arq.rs` |
...
| 12 | **README status table is stale/wrong** ("OFDM Partial… sync has CFO limitations", "QPSK/8PSK/QAM not wired to engine", "Channel prediction EWMA", "LDPC 6 rates"). A sysop evaluating the project will bounce. | spec | M | S | README.md:10-40 vs SPEC §6 |
...
| 16 | **There is no RX-to-TX turnaround delay at all -- a correctness defect, not just an unmeasured latency.** The receive arm calls `transmit_samples` for the ACK immediately once `ack_info` is computed (`event_loop.rs:1330-1339`); no production code reads `turnaround_ms`. The sending peer's own PTT-release timer keeps it keyed for `audio_duration_ms + drain_ms + tail_delay_ms` after its last sample (`event_loop.rs:1561-1584`), so on real half-duplex RF the ACK can go out while the peer is still transmitting and deaf to it. Audio poll is 20 ms, retransmit poll 500 ms, PTT pre 50 ms + tail 200 ms, ACK is a full >= 0.39-1.365 s frame at the *data* level. Implement and verify an actual turnaround delay, and add a short dedicated ACK frame. | correctness | H | M | `event_loop.rs:647-654,1330-1339,1561-1584`; `coppad.toml.example`; `arq.rs:82` `DEFAULT_TURNAROUND 150 ms` is the *model*, not enforced |
| 17 | **`turnaround_ms: 500` is dead config; `Profile.arq_window/max_payload` are unenforced, not unread** -- the CLI's `cmd_config` reads and prints both as the user's current profile limits, but the engine/daemon ARQ configuration ignores them and uses `ArqConfig::default()` (window 8, RTO 5 s, 150 ms) regardless, so the CLI actively shows stale, presentation-only numbers. Either wire these into the engine/daemon or stop displaying limits that aren't enforced. (`Profile.ofdm_profile` is a separate, narrower gap: it IS read at construction but ignored by `select_ofdm_profile` on a later speed-level change -- see item 2.) | robustness | M | S | `event_loop.rs:174,357` (fields unenforced); `event_loop.rs:234,1386` `ArqConfig::default()`; `profiles.rs`; `coppa-cli/src/main.rs:935-936` (CLI display) |
...
| 32 | **`cargo bench`'s end-to-end OFDM benches (`engine_encode`/`engine_decode`) only cover one speed level** (the default), alongside legacy BPSK/Viterbi/FFT primitive micro-benches; add a multi-level sweep, a streaming/daemon-path latency bench, and an OTA goodput bench. | test-gap | L | S | `benches/throughput.rs` |
...
| 34 | **Station ID has a real startup gap.** `transmit_samples` (the single TX chokepoint) correctly prepends the ID/beacon whenever `id_due()` is true, and a completely silent session that never transmits correctly never IDs -- that much is by design and fine per Part 97.119. But `last_id_time` initializes to `Instant::now()` at daemon startup and `id_due()` stays false until the full interval elapses, so any real transmission in the first ~9 minutes after startup goes out with no ID at all. Fix: force ID on the first transmission regardless of the timer. | safety | H | S | `event_loop.rs:180,358,1003-1010,1529-1545,1644-1658` |
...
1. **Two-host audio-cable loopback (the standard bench test).** Two computers, two USB soundcards, TRS cables TX→RX both ways, no radio. Run `coppad` on each with `arq_enabled = true`, `ptt_method = "none"`. (a) Decode golden WAVs played through the cable: realistic target **18/20, not 20/20**. Note 1: the 20 vectors do not all use the same OFDM profile at a given level -- the generator routes levels 5/6/9 to `vhf_wide` for clean/AWGN vectors but forces `hf_standard` for those same levels on the Poor/SSB+CFO vectors (`golden_vectors.rs:61-77`), so a single fixed `coppad` config cannot decode all 20 without reconfiguring between profile groups. Either run this in two passes (one `coppad` config per profile group) or use a manifest-driven harness that selects each vector's recorded profile. Note 2: `L1_awgn12`/`L2_awgn12` decode fine via the batch API but produce zero frames via the daemon's actual `StreamingReceiver::push_samples` path -- a pre-existing, already-documented gap (`crates/coppa-cli/tests/rx_golden.rs`) rooted in item 20's unfiltered-sync-detector margin, deferred to Phase 2 (H-102). Don't treat these two as a Phase 1 blocker. (b) 10-minute ARQ session, fixed L2 then RateLoop: **0 drops, goodput ≥ 90 % of the shipped daemon's per-frame-ACK ceiling at L2 on a clean channel (≈ 265 bps -- see the corrected throughput model above; the 550 bps figure in an earlier draft of this report assumed batched ACKs the daemon does not send)**. (c) Repeat with a −50 ppm / +50 ppm software resample on one side (or two cards of different make): **0 drops**. (d) Measure turnaround with a GPIO/scope tap on BOTH stations' PTT lines, not just RX-last-sample -> local-PTT-assert. Record the far-end's actual PTT-off edge (last audio sample + `drain_ms` + `tail_delay_ms`, per H-104) and this station's PTT-on edge for the ACK. **Pass requires BOTH a minimum guard (this station's PTT-on must be AFTER the far end's measured PTT-off edge, with margin -- a result of 0 ms or a negative gap means H-104 is not actually fixed even if fast) AND a maximum latency bound (target ≤ 250 ms so turnaround doesn't dominate throughput)**. A single `≤ 250 ms` ceiling is insufficient by itself: the daemon's current zero-turnaround behavior would trivially pass a ceiling-only check while still exhibiting H-104's exact collision. This stage also settles whether item 2's `vhf_wide` frames pass a real codec's anti-alias filter.
...
Until stage 1 is done, the honest one-line status for a sysop is: *"Simulated only; SSB-realistic ladder tops out at ≈0.48 kbps user at the daemon's shipped one-ACK-per-frame default; does not yet hold a link through CCIR-Moderate fading."*
...
Reading: no level clears FER ≤ 10 % with a Wilson bound at any SNR; the floors are SNR-independent from ≈ 18 dB, i.e. pure fading outage that more SNR cannot buy back (the fade-diversity gap, item 3). The best *goodput* on Poor at any SNR is ≈ 0.9–1.0 kbps PHY (levels 3–4 at ≈ 25–50 % FER) — before ARQ retransmission cost and before turnaround, versus VARA-2300's ≈ 1.5 kbps and PACTOR-4's ≈ 2.9 kbps *net delivered* at 20 dB MPP in the IONOS study, both with zero link drops. Below 12 dB, sync detection is only the dominant failure at levels 1–2 (16/2 and 15/13 sync/LDPC failures respectively at 6 dB); from level 3 up, LDPC non-convergence already dominates at 6 dB (e.g. level 3: 12 sync vs 18 LDPC; level 9: 15 sync vs 35 LDPC) — item 20's unfiltered-detector margin is real for the two most robust levels, not the whole ladder.
...
Reading: on the CCIR-Moderate channel a real SSB rig should expect — the ARRL/Winlink "typical" channel — coppa's usable ladder is levels 1–3 (BPSK/QPSK 1/2, ≈ 0.3–1.0 kbps PHY at ≤ 16 % FER) from 18 dB; everything from QPSK 3/4 up sits on a 35–100 % SNR-independent outage floor. Best PHY goodput at any SNR on Moderate is ≈ 1.2 kbps (L4 at 34 % FER), against VARA-2300 MPG ≈ 1.6 kbps and PACTOR-4 ≈ 2.9 kbps *net* at 20 dB with no drops. Failure mode is LDPC non-convergence at levels 2 and up from 18 dB (a per-frame fade the code/interleaver cannot cover — item 3); level 1 is the exception -- its 18 dB point is a lone sync failure with zero LDPC failures, and even at 30 dB it splits 1 sync/1 LDPC, so level 1's floor is not purely an LDPC story. Below 12 dB, sync detection is only the dominant failure at levels 1-2 (matching the Poor-channel finding above); from level 3 up, LDPC failures already dominate at 6 dB too (e.g. level 4: 10 sync vs 33 LDPC; level 9: 15 sync vs 35 LDPC) -- item 20's unfiltered-detector margin is real for the two most robust levels, not the whole ladder.
