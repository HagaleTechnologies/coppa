...
| 17 | **`turnaround_ms: 500` and `Profile.arq_window/max_payload` are dead config** — the daemon uses `ArqConfig::default()` (window 8, RTO 5 s, 150 ms) regardless. (`Profile.ofdm_profile` is a separate, narrower gap: it IS read at construction but ignored by `select_ofdm_profile` on a later speed-level change -- see item 2.) | robustness | M | S | `event_loop.rs:174,357` (fields never read); `event_loop.rs:234,1386` `ArqConfig::default()`; `profiles.rs` |
