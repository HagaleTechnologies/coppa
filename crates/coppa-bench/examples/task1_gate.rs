//! Phase 2 Task 1 bench gate: paired sweeps at levels 2 and 5, watterson-{moderate,poor}
//! + awgn, 400 trials per SNR point.
//!
//! Prints a markdown table plus the FER@10%-CI threshold per (level, channel), and
//! writes raw CSV to `results/task1-gate/`.
use std::fs;
...
fn main() {
    ...
    let out_dir = PathBuf::from("results/task1-gate");
