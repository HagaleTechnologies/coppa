//! Coarse-delay drift Kalman tracker gate bench: same paired sweep as
//! task1_gate.rs/task7_gate.rs (levels 2 and 5, watterson-{moderate,poor} +
//! awgn, 400 trials per SNR point, same seed), so the numbers are directly
//! comparable to results/p1-hotfix/{moderate,poor,awgn}.csv (pre-Task-1
//! baseline, 18 dB), Task 1's regressed 24 dB, and Task 7's regressed 30 dB.
//!
//! DRIFT_TRACKER_ENABLED in coppa_modem.rs selects which payload path this
//! measures — run once with it false (sanity-check against the known 30 dB
//! Task 7 number) and once true (this design), per Task 5.
//!
//! Prints a markdown table plus the FER@10%-CI threshold per (level, channel),
//! and writes raw CSV to results/drift-tracker-gate/.
use std::fs;
...
fn main() {
    ...
    let out_dir = PathBuf::from("results/drift-tracker-gate");
